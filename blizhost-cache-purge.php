...
Copyright 2017-2024: Blizhost (email: contato@blizhost.com), Mika A. Epstein (email: ipstenu@halfelf.org) - based on version 4.0.2
...
class BlizCloudCachePurger {
	...
	public $p_version = '4.0.6'; // Version
	...
	public function __construct( ) {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'load_blizhost_logo_adminbar' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'load_blizhost_logo_adminbar' ) );
	...
	/**
	 * Load Blizhost Logo Admin Bar Style
	 */
	public function load_blizhost_logo_adminbar() {
	...
	public function bliz_cdn_imageurl_replace($url) {
		...
		$host = str_replace("www.", "", $site_url['host']);
		...
		if (!preg_match("/(https?:)?\/\/(www.)?".$hostregex."\/((?![\"']).)*\.(jpe?g|gif|png|webp)($|\?.*)/i", $url)) {
			return $url;
		}
		...
		$dslash_pos = strpos($url, '//') + 2;
		$src_pre  = substr($url, 0, $dslash_pos); // http:// or https://
		$src_post = substr($url, $dslash_pos); // The rest after http:// or https://
	...
	public function bliz_cdn_imageurl() {
		...
		if (defined('DOING_AJAX') || defined('DOING_CRON') || is_admin() || (isset($GLOBALS['pagenow']) && $GLOBALS['pagenow'] === 'wp-login.php')) {
			return;
		}
	...
	public function blizexecutePurge() {
		...
		if ( empty($purgeUrls) ) {
			if ( isset($_GET['bliz_flush_all']) ) {
				$this->blizpurgeUrl( $http_proto . $host .'/?bliz-regex' ); // Clears the main domain
			}
		} else {
	...
	public function blizpurgeUrl($url) {
		...
		if (isset($dir_exp[1]) && isset($dir_exp[2])) {
			$directory = $dir_exp[1];
			$user = $dir_exp[2];
		} else {
		...
		if (file_exists($du_api) && is_readable($du_api)) {
			$api_key = file_get_contents($du_api);
			if ($api_key === false) {
				$api_key = '';
			}
		} else {
			...
			// Attempt to write the new API key to the file
			if (file_put_contents($du_api, $api_uniqid) !== false) {
				$api_key = $api_uniqid;
			} else {
				$api_key = '';
			}
		...
		// If the API key is empty, do not proceed
		if (empty($api_key)) {
			return false;
		}
	...
	public function purgeNoID( ...$args ) {
		...
		array_push($listofurls, self::the_home_url().'/?bliz-regex' );
